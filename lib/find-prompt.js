...
exports.makeFindPrompt = function (p, projectRoot) {
    return function findPrompt(object, dir, sumanOptions, cb) {
        ...
        k.stdout.pipe(JSONStdio.createParser()).on(JSONStdio.stdEventName, function (obj) {
            if (obj && obj.file) {
                files.push(obj.file);
            }
            else {
        ...
        k.stderr.pipe(prepend_transform_1.pt(chalk.yellow(' [suman "--find-only" process stderr] '), { omitWhitespace: true })).pipe(process.stderr);
